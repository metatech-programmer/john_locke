...
function dificultad() {
  switch (this.id) {
    case "facil":
      generarCartas(4, 8, filosofia);
      break;
    case "medio":
      generarCartas(6, 18, filosofia_politica);
      break;
    case "dificil":
      generarCartas(8, 32, john_locke);
      break;
  }
...
function carta(contenedor, img, numImg) {
  ...
  imgReverso.setAttribute("src", "img/reverso.webp");
...
function mezclarImagenes(imagenes, numImg) {
  ...
  while (i < baraja.length) {
    ...
    if (
      !baraja.includes(nuevaImagen) ||
      contarRepeticiones(baraja, nuevaImagen) < 2
    ) {
...
function comprobarParejas() {
  if (numImgVisibles == 2) {
    ...
    for (let i = 0; i < imagenes.length; i++) {
      if (
        !parejasAcertadas.includes(imagenes[i].getAttribute("src")) &
        (imagenes[i].getAttribute("visible") == "true")
      ) {
    ...
    if (parejas[0].getAttribute("src") != parejas[1].getAttribute("src")) {
      ...
      setTimeout(function () {
        ...
        setTimeout(function () {
          ...
          document.getElementById("mdPregunta").innerText =
            preguntasRespuestasJohnLocke[numAleatorio].pregunta;
          document.getElementById("modalPregunta").style.backdropFilter =
            "blur(2px)";
        }, 1000);
      }, 1000);
    } else {
...
function bloquearPanel(bloquear) {
  ...
  if (bloquear) tablero.classList.add("bloquear");
  else tablero.classList.remove("bloquear");
  ...
  intro.setOptions({
    steps: [
      ...
      },
      {
        ...
        intro:
          "Visualiza la tabla de puntuaciones guardadas, en orden descendente.",
      },
      {
        element: "#numPartidas",
